...
public class ConfigFileOperation {
    ...
    public String getGroupAvatarFileDir(){
        String protectDir = "";
        InputStream in = this.getClass().getResourceAsStream("/group_avatar_file_dir.properties");
        Properties properties = new Properties();
        try {
            InputStreamReader inputStreamReader = new InputStreamReader(in, "UTF-8");
            properties.load(inputStreamReader);
            inputStreamReader.close();
            in.close();
            protectDir = properties.getProperty("group_avatar");
        } catch (FileNotFoundException e) {
            e.printStackTrace();
        } catch (IOException e) {
            e.printStackTrace();
