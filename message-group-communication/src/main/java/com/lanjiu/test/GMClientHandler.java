...
import static com.lanjiu.test.GMClient.channelLoginRegistrationGroup;


public class GMClientHandler extends SimpleChannelInboundHandler<BusinessProtocolMessageStandard.CheckUnifiedEntranceMessage> {

    private final Logger log = LoggerFactory.getLogger(GMClientHandler.class);
    //线程安全心跳失败计数器
...
//        System.out.println("测试响应："+msg);
        log.info("群服务响应数据：" + msg);
        log.info("群服务响应数据类型：" + msgType);
        log.info("群服务响应数据管道ID : " + ctx.channel().id().toString());
        log.info("群服务响应数据请求用户ID : " + head.getFromId());
        return;
