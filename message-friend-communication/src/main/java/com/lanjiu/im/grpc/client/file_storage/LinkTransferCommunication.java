...
import com.lanjiu.im.grpc.utils.ConstantsContent;
import com.lanjiu.im.grpc.utils.DeleteFileUtil;
...
public class LinkTransferCommunication {
    ...
    public boolean storageFileIsSuccessFulWithGroup(GroupTransmissionRequest request){
        ...
        System.out.println("settingsType:" + settingsType);
        ...
        System.out.println("registeredUserId:" + registeredUserId);
        System.out.println("-------------------------------------------");
        //调用文件存储服务器，保存对应文件记录
    ...
    public FriendsTransmissionResponse deleteFileInformationWithFriend(FriendsTransmissionRequest request) {
        blockingStub = linkTransferCommunication();
        FriendsTransmissionResponse response = null;
        blockingStub.deleteFileInformationWithFriend(request);
        try {
            TransferStorageClient.shutdownNow();
        } catch (InterruptedException e) {
            e.printStackTrace();
