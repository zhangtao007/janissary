...
public class JCRC32 {
    ...
    private static final Logger logger = LoggerFactory.getLogger(JCRC32.class);
    /**
     * 编码
     *
     * @param data
    ...
    //数据包类型判断
    public String packageProtocolJudge(BusinessProtocolMessageStandard.UnifiedEntranceMessage msg) {
...
//        Logger.print(protocolHexString);
        return protocolHexString;
    }

    //给数据包加校验信息
    public BusinessProtocolMessageStandard.CheckUnifiedEntranceMessage packageCheckSum(BusinessProtocolMessageStandard.UnifiedEntranceMessage msg) {
...
//        logger.info("packageCheckSum: ::msg:{}",msg);
        BusinessProtocolMessageStandard.CheckUnifiedEntranceMessage checkUnifiedEntranceMessage = BusinessProtocolMessageStandard.CheckUnifiedEntranceMessage.newBuilder()
                .setUnifiedEntranceMessage(msg)
                .setChecksum(checksum_Translate).build();
        return checkUnifiedEntranceMessage;
    }

    //checksum的自定义校验函数
    public CheckSumResult checkSumValidateProtocol(BusinessProtocolMessageStandard.CheckUnifiedEntranceMessage checkUnifiedEntranceMessage, BusinessProtocolMessageStandard.UnifiedEntranceMessage msg) {

        String checksum = checkUnifiedEntranceMessage.getChecksum();
//        Logger.print("checkSumValidateProtocol" + checksum);
        String protocolHexString = packageProtocolJudge(msg);
